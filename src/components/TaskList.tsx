...
const TaskList: React.FC<TaskListProps> = ({ todos }) => {
  ...
  const [inProgressList, setInProgressList] = useState(filterTasks(tasks, 'In Progress'));
  ...
  const handleStop = (e: MouseEvent, data: DraggableData, task: Task) => {
    ...
    if (status === 'To Do') {
      ...
      if (x > 0) {
        setInProgressList(inProgressList.filter((todo) => todo.id !== task.id));
        updateTask(task, 'Done');
        setDoneList([task, ...doneList]);
      } else {
  ...
  return (
    ...
    >
      <GridItem
        sx={isStarted ? borderStyle : {}}
      >
        <TaskListColumn tasks={todoList} title="To Do" handleDrag={handleDrag} handleStart={handleStart} 
      ...
      <GridItem
        sx={isStarted ? borderStyle : {}}
      >
        <TaskListColumn tasks={inProgressList} title="In Progress" handleDrag={handleDrag} handleStart={handleStart} handleStop={handleStop} />
      </GridItem>
      <GridItem
        sx={isStarted ? borderStyle : {}}
      >
        <TaskListColumn tasks={doneList} title="Done" handleDrag={handleDrag} handleStart={handleStart} handleStop={handleStop} />
      </GridItem>
    </Grid>
  );
};
